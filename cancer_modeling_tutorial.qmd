...
time_H_P <- rweibull(500, shape = 3, scale = 1.3)
...
Let's run through the steps for modeling. The functions below are in 'R/decision_model_functions.R' if you want to take a closer look. Each function generates variables of the form "time_X_Y", denoting the time from state X to state Y. See if you can find and fix the error hiding in one of the following functions: 'simulate_cancer_progression()', 'simulate_cancer_mortality()', 'calc_mortality_outcomes()'.
...
# Set number of parameter sets to sample - feel free to change
n_samp <- 500
