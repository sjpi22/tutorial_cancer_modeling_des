...
n_mort_ages <- length(l_params_all$d_time_H_Do$female$params$xs)
df_mort <- data.frame(sex = c(rep("Female", n_mort_ages),
                              rep("Male", n_mort_ages)),
                      yrs = c(l_params_all$d_time_H_Do$female$params$xs,
                              l_params_all$d_time_H_Do$male$params$xs),
...
for (stg in l_params_all$v_cancer) {
  n_surv_ages <- length(l_params_all$d_time_C_Dc[[stg]]$params$xs)
  temp_df_surv <- data.frame(stage = toupper(stg),
                             yrs = l_params_all$d_time_C_Dc[[stg]]$params$xs,
...
res_min <- params_to_calib_targets(l_params_all, prior_map$min, param_map,
                                   v_ages = v_ages)
res_max <- params_to_calib_targets(l_params_all, prior_map$max, param_map,
                                   v_ages = v_ages)
out_calib_targets <- rbind(res_min, res_max)
...
```{r}
# File paths 
path_imabc <- "output/calibration/IMABC/IMABC_outputs.rds"

# Set random seed
set.seed(l_params_all$seed)

# Reshape true targets and get mapping
l_true_reshaped <- reshape_calib_targets(l_true_targets, output_map = TRUE)

# Load IMABC results
IMABC_outputs <- readRDS(path_imabc)

# Load true parameters (unknown in real life!)
true_params <- readRDS("ground_truth/true_param_map.rds") # Output: param_map

# Sample with replacement
n_imabc_sample <- nrow(IMABC_outputs$good_parm_draws) * 100
indices_imabc_sample <- sample(1:nrow(IMABC_outputs$good_parm_draws), 
                               n_imabc_sample, 
                               replace = TRUE, 
                               prob = IMABC_outputs$good_parm_draws$sample_wt)
```
...
imabc_params_unweighted <- IMABC_outputs$good_parm_draws %>%
...
# IMABC posterior targets
imabc_targets_unweighted <- IMABC_outputs$good_sim_target %>%
...
collapse_mean  <- summaryBy( . ~ index , FUN=c(mean), data=imabc_targets, keep.names=TRUE)
...
# Get confidence interval of true targets
output_ci <- calc_target_ci(l_true_targets)

out_summary <-  data.frame(l_true_reshaped$target_map,
                           true_val = l_true_reshaped$v_targets,
                           true_lb = output_ci$lb,
                           true_ub = output_ci$ub,
