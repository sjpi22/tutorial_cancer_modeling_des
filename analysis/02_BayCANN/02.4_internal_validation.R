...
# Load calibrated parameters and outputs
m_param_samp <- read_csv(file_posterior) %>%
  dplyr::select(-lp) # Remove last non-parameter column
m_outputs <- readRDS(file_outputs)

colnames(m_param_samp) <- df_true_params$var_id
df_params_long <- m_param_samp %>%
...
# Summarize targets
collapse_mean  <- colMeans(m_outputs)
