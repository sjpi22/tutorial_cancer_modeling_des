...
df_fn_grps <- l_params_calib$df_target %>%
  ...
  mutate( # Assign activation functions for output layer
    activation = ifelse(target_groups %in% v_groups_categorical, "softmax", # Categorical targets that sum to 1
  ...
  mutate(
    ...
    fn_grp_label = ifelse( # Assign labels to define groups within the same output head
...
for (grp in unique(df_fn_grps$fn_grp)) {
  ytrain_scaled_reshape <- c(ytrain_scaled_reshape, 
                             list(ytrain_scaled[, l_params_calib$df_target$target_groups %in% df_fn_grps$target_groups[df_fn_grps$fn_grp == grp]]))
  ytest_scaled_reshape <- c(ytest_scaled_reshape, 
                            list(ytest_scaled[, l_params_calib$df_target$target_groups %in% df_fn_grps$target_groups[df_fn_grps$fn_grp == grp]]))
}
...
# Verify that the Stan file produces the correct data if necessary
